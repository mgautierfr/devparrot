...
class EditSection:
	...
	def createMenu(self):
		self.menu.add_command(label='Undo', command=lambda:controler.run_command('undo'))
		self.menu.add_command(label='Redo', command=lambda:controler.run_command('redo'))
		self.menu.add_separator()
		self.menu.add_command(label='Copy',  command=lambda:controler.run_command('copy'))
		self.menu.add_command(label='Cut',   command=lambda:controler.run_command('cut'))
	...
	def postCommand(self):
		...
		try:
			import capi
			capi.currentDocument.get_currentView().view.index('sel.first')
			capi.currentDocument.get_currentView().view.index('sel.last')
